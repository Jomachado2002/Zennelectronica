...
async function testImageLoadPerformance() {
    try {
        ...
        for (let i = 0; i < Math.min(5, products.length); i++) {
            ...
            for (let j = 0; j < product.productImage.length; j++) {
                ...
                try {
                    ...
                    console.log(`  📸 Imagen ${j + 1}: ${loadTime}ms (${imageInfo.contentType})`);
            ...
            console.log(`  📊 Promedio: ${productResults.stats.avgLoadTime}ms (JPG: ${productResults.stats.avgJpgLoadTime}ms, WebP: ${productResults.stats.avgWebpLoadTime}ms)\n`);
        }
        ...
        const filename = `image_performance_test_${timestamp}.json`;
