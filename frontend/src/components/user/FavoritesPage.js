...
const FavoritesPage = ({ user, onNavigate }) => {
  ...
  const loadFavorites = () => {
    try {
      const savedFavorites = localStorage.getItem(`favorites_${user?.id || user?._id || 'guest'}`);
      if (savedFavorites) {
  ...
  const removeFavorite = (productId) => {
    try {
      ...
      localStorage.setItem(`favorites_${user?.id || user?._id || 'guest'}`, JSON.stringify(updatedFavorites));
    } catch (error) {
      ...
      {favorites.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {favorites.map((product) => (
            <div key={product.id} className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-lg transition-shadow border">
              {/* Imagen del producto */}
